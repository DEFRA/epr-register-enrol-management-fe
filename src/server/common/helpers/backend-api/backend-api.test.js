...
describe('#addWorkItemTaskNote (RA-129)', () => {
  test('POSTs the note text body to the task-scoped notes endpoint with user headers', async () => {
    const workItem = {
      id: 'abc',
      notes: [
        {
    ...
    const { addWorkItemTaskNote } = await import('./backend-api.js')
    const result = await addWorkItemTaskNote({
      workItemId: 'abc',
      taskId: 'check eligibility',
      text: 'hello',
      baseUrl: 'http://backend:8085/',
    ...
    expect(fetchImpl).toHaveBeenCalledWith(
      'http://backend:8085/work-items/abc/tasks/check%20eligibility/notes',
      expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({ text: 'hello' }),
        headers: expect.objectContaining({
  ...
  test('Surfaces a 400 problem with the engine reason', async () => {
    const fetchImpl = vi.fn().mockResolvedValue({
      ok: false,
      status: 400,
      json: () => Promise.resolve({ detail: 'Note text is required.' })
